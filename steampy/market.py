...
class SteamMarket:
    ...
    def create_sell_order(self, assetid: str, game: GameOptions, money_to_receive: str) -> dict:
        ...
        headers = {'Referer': "%s/profiles/%s/inventory" % (SteamUrl.COMMUNITY_URL, self._steam_guard['steamid']),
            'Origin': f'{SteamUrl.COMMUNITY_URL}/profiles/{self._steam_guard["steamid"]}/inventory'}

        response = self._session.post(SteamUrl.COMMUNITY_URL + "/market/sellitem/", data, headers=headers)
        response = response.json()

        if not response.get('success'):
            raise Exception('unsuccessfull sell')

        message = response.get('message')

        if response.get("needs_mobile_confirmation") or (message is not None and 'pending confirmation' in message):
    ...
    @login_required
    def get_trade_history(self, start: int, count: int) -> tuple[int, dict, str]:
        """
        returns (total_count, assets, html)
        """
        url = f'https://steamcommunity.com/market/myhistory/render/?query=&count={count}&start={start}'
        reponse = self._session.get(url).json()

        if reponse.get('success'):
            total_count = reponse.get('total_count')
        else:
            return (None, None, None)

        return (total_count, reponse.get('assets'), reponse.get('results_html'))
    ...
    def cancel_sell_order(self, sell_listing_id: str) -> None:
        data = {"sessionid": self._session.cookies.get_dict()['sessionid']}

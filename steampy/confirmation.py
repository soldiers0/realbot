...
class ConfirmationExecutor:
    ...
    def _get_confirmations(self) -> List[Confirmation]:
        confirmations = []
        confirmations_json: dict = json.loads(self._fetch_confirmations_page().text)

        for confirmation_div in confirmations_json['conf']:
            _id = confirmation_div['creator_id']
            data_confid = confirmation_div['id']
            data_key = confirmation_div['nonce']
    ...
    def _fetch_confirmation_details_page(self, confirmation: Confirmation) -> str:
        tag = 'details'
        params = self._create_confirmation_params(tag)
        response = self._session.get(self.CONF_URL + '/detailspage/' + confirmation.data_confid, params=params)
    ...
    def _get_confirmation_trade_offer_id(confirmation_details_page: str) -> str:
        soup = BeautifulSoup(confirmation_details_page, 'html.parser')
        full_offer_id = soup.find('div', {"class": "tradeoffer"})['id']
